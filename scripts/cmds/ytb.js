...
module.exports = {
  config: {
    ...
    role: 0,
    description: {
      en: "Search & download YouTube audio or video using your new API",
    },
    category: "media",
    guide: {
  ...
  onStart: async ({ api, args, event }) => {
    ...
    try {
      ...
      if (videos.length === 0) {
        return api.sendMessage(`⭕ No results for: ${searchTerm}`, event.threadID, event.messageID);
      }
      ...
      api.sendMessage(
        {
          ...
          attachment: await Promise.all(
            videos.map((v) =>
              downloadThumbnail(v.thumbnail, path.join(tmp, `thumb_${v.videoId}.jpg`))
            )
          ),
        },
        event.threadID,
      ...
      console.error(err);
      api.sendMessage("❌ YouTube search failed.", event.threadID, event.messageID);
    }
  },
  ...
  onReply: async ({ event, api, Reply }) => {
    ...
    try {
      ...
      const filePath = path.join(
        cacheDir,
        Reply.isAudio ? `ytb_audio_${selected.videoId}.mp3` : `ytb_video_${selected.videoId}.mp4`
      );

      await downloadFile(data.download_url, filePath);
      await new Promise(res => setTimeout(res, 500)); // safety delay
      ...
      await api.sendMessage(
        ...
        event.threadID,
        () => fs.unlink(filePath, () => {}), // auto delete after send
        event.messageID
      );
    } catch (err) {
      console.error(err);
      api.sendMessage(`❌ Download failed: ${err.message}`, event.threadID, event.messageID);
    }
  },
};
